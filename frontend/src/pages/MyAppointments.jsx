        ...
        {appointments.map((item, index) => (
          <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b'>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {item.cancelled && (
                <button className='sm:min-w-48 py-2 border border-red-500 rounded text-red-500 text-sm'>Appointment cancelled</button>
              )}
              {item.isCompleted && (
                <button className='sm:min-w-48 py-2 border border-green-500 rounded text-green-500 text-sm'>Completed</button>
              )}
            </div>
          </div>
        ))}
